...
class ListingsAPI extends PersonalizationAPI {
  ...
  getFloorplanByListingId = async (listingId: string): Promise<_.IListingPhotosResponse> => {
    return this.get(`/listings/floorplan/${listingId}`).catch(err => new ApolloError(toError(err)));
  };

  getMenuByListingId = async (listingId: string): Promise<_.IListingPhotosResponse> => {
    return this.get(`/listings/menu/${listingId}`).catch(err => new ApolloError(toError(err)));
  };
  ...
  getGoogleReviews = async (placeId: string): Promise<Array<any>> => {
    return this.get(`${config.GOOGLE_PLACE_API_URL}${placeId}&fields=rating,reviews&key=${config.GOOGLE_MAP_API}`).catch(err => new ApolloError(toError(err)));
  };
  ...
  createInspection = async (listingId: any, messageId: any, guestId: any, date: any, time: any) => {
    return this.post("/inspection", { listingId, messageId, guestId, date, time }).catch(err => new ApolloError(toError(err)));
  };
